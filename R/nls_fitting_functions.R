...
#----------------------------------------------------------------------------------------------------------------
#----------------------------------------------------------------------------------------------------------------
nls_fitting_function_tree <- function(species,region,params,tree,plotson=F...){
...
#----------------------------------------------------------------------------------------------------------------
#----------------------------------------------------------------------------------------------------------------
#- Function for fitting bark-------------------------------------------------------------------------------------
nls_fitting_function_bark <- function(species,region,params,tree,plotson=F...){
  ...
  #Merging both data bases
  sample_species<-merge(stem_sample, params_species, by='SPCD')
  
  #Filtering the columns of interest ST_BK_CV_TOT BARK CUBIC FOOT VOLUME
  sample_species2<-sample_species %>% 
    select(SPCD,AUTHOR,TREENO,DO_BH,HT_TOT,ST_BK_CV_TOT,DIVISION,model,a,b,c)
  
  #- make plots if requested. Model diagnostic plots may be useful too.
  if(plotson==T){
    plot(ST_BK_CV_TOT~HT_TOT, data=sample_species2)
    plot(ST_BK_CV_TOT~DO_BH, data=sample_species2)
  }
  
  #- Fit the Schumacher-Hall model
  nls_fitbark<-nls(ST_BK_CV_TOT~a*DO_BH^b*HT_TOT^c, data = sample_species2,
                   start = list(a=sample_species2$a[1],b=sample_species2$b[1],c=sample_species2$c[1]))
  return(nls_fitbark)
}
#----------------------------------------------------------------------------------------------------------------
...
#- Function for fitting branch -----------------------------------------------------------------------------------

nls_fitting_function_branch <- function(species,region,params,tree,plotson=F...){
  ...
  #Filtering the columns of interest
  sample_species2<-sample_species %>% 
    select(SPCD,AUTHOR,TREENO,DO_BH,HT_TOT,REGION,BRT_WDBK_DW_TOT,DIVISION,model,a,b,c)
  
  #- make plots if requested. Model diagnostic plots may be useful too.
  if(plotson==T){
    plot(BRT_WDBK_DW_TOT~HT_TOT, data=sample_species2)
    plot(BRT_WDBK_DW_TOT~DO_BH, data=sample_species2)
  }
  
  #- Fit the Schumacher-Hall model
  nls_fitBranch<-nls(BRT_WDBK_DW_TOT~a*DO_BH^b*HT_TOT^c, data = sample_species2,
                     start = list(a=sample_species2$a[1],b=sample_species2$b[1],c=sample_species2$c[1]))
  return(nls_fitBranch)
}
#----------------------------------------------------------------------------------------------------------------
#----------------------------------------------------------------------------------------------------------------
